...
func (m *MenuHandler) createConfig() {
	utils.LogMessage(0, 0, "Creating new config...", "info")
	...
	default:
		utils.LogMessage(0, 0, "Invalid choice, using AntiCaptcha by default", "warning")
		captchaUsing = "antiCaptcha"
		fmt.Print(color.CyanString("Enter your AntiCaptcha API key: "))
		apiKey, _ := reader.ReadString('\n')
		antiCaptchaApikey = []string{strings.TrimSpace(apiKey)}
	...
	if err != nil {
		utils.LogMessage(0, 0, "Failed to create config: "+err.Error(), "error")
		return
	}

	err = os.WriteFile("config.json", file, 0644)
	if err != nil {
		utils.LogMessage(0, 0, "Failed to save config: "+err.Error(), "error")
		return
	}

	utils.LogMessage(0, 0, "Config file created successfully!", "success")
}
...
func (m *MenuHandler) EditConfig() {
	...
	case "1":
		...
		default:
			utils.LogMessage(0, 0, "Invalid choice, no changes made", "error")
			return
		}
	case "2":
		if config.CaptchaServices.CaptchaUsing == "antiCaptcha" {
	...
	file, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		utils.LogMessage(0, 0, "Failed to update config: "+err.Error(), "error")
		return
	}
	...
	utils.LogMessage(0, 0, "Config updated successfully!", "success")
